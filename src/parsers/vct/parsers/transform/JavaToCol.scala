...
case class JavaToCol[G](
  ...
  def convert(
      implicit unit: CompilationUnitContext
  ): Seq[GlobalDeclaration[G]] =
    unit match {
      case CompilationUnit0(pkg, imports, decls, _) =>
        Seq(new JavaNamespace(
          pkg.map(convert(_)),
          imports.map(convert(_)),
          decls.flatMap(convert(_)),
        ))
    }

  def convert(implicit pkg: PackageDeclarationContext): JavaName[G] =
    pkg match { case PackageDeclaration0(_, _, name, _) => convert(name) }

  def convert(implicit imp: ImportDeclarationContext): JavaImport[G] =
    imp match {
      case ImportDeclaration0(_, isStatic, name, star, _) =>
        JavaImport(isStatic.nonEmpty, convert(name), star.nonEmpty)
    }

  def convert(
      implicit decl: TypeDeclarationContext
  ): Seq[GlobalDeclaration[G]] =
    decl match {
      case TypeDeclaration0(
            mods,
            ClassDeclaration0(
              contract,
              _,
      ...
      case TypeDeclaration1(
            ...
            EnumDeclaration0(
              ...
              Some(constants),
              _,
              None | Some(EnumBodyDeclarations0(_, Seq())),
              _,
            ),
          ) =>
      ...
      case TypeDeclaration2(
            ...
            InterfaceDeclaration0(
              ...
              args,
              ext,
              InterfaceBody0(_, decls, _),
            ),
          ) =>
        Seq(new JavaInterface(
          convert(name),
          mods.map(convert(_)),
          args.map(convert(_)).getOrElse(Nil),
          ext.map(convert(_)).getOrElse(Nil),
          decls.flatMap(convert(_)),
        ))
      case TypeDeclaration3(
            mods,
            AnnotationTypeDeclaration0(
              _,
              _,
              name,
              AnnotationTypeBody0(_, decls, _),
            ),
          ) =>
        Seq(new JavaAnnotationInterface(
          convert(name),
          mods.map(convert(_)),
          Java.JAVA_LANG_ANNOTATION_ANNOTATION,
          decls.flatMap(convert(_)),
        ))
  ...
  def convert(implicit constants: EnumConstantsContext): Seq[EnumConstant[G]] =
    constants match {
      case EnumConstants0(constant) => Seq(convert(constant))
      case EnumConstants1(constants, _, constant) =>
        convert(constants) :+ convert(constant)
    }

  def convert(implicit constant: EnumConstantContext): EnumConstant[G] =
    constant match {
      case EnumConstant0(Nil, name, None, None) =>
        new EnumConstant()(origin(constant).sourceName(convert(name)))
      case constant =>
        fail(
          constant,
          "Arguments, annotations, or class body on enum alternatives not supported",
        )
    }

  def convert(
      implicit decl: AnnotationTypeElementDeclarationContext
  ): Option[JavaAnnotationMethod[G]] =
    decl match {
      case AnnotationTypeElementDeclaration0(
            mods,
            AnnotationTypeElementRest0(
              returnType,
              AnnotationMethodOrConstantRest0(
                AnnotationMethodRest0(name, _, _, default)
              ),
              _,
            ),
          ) =>
        ...
        if (
          !modifiers.subsetOf(Set(
            JavaPublic()(DiagnosticOrigin),
            JavaAbstract()(DiagnosticOrigin),
          ))
        ) {
          fail(
            decl,
            "Only modifiers allowed on @interface members are public, abstract",
          )
        }
        Some(new JavaAnnotationMethod(
          convert(returnType),
          convert(name),
          default.map(convert(_)),
        ))
      case AnnotationTypeElementDeclaration1(_) => None
    }

  def convert(implicit default: DefaultValueContext): Expr[G] =
    default match {
      case DefaultValue0(_, ElementValue1(expr)) => convert(expr)
      case DefaultValue0(_, elementValue) => ??(elementValue)
    }

  def convert(implicit modifier: ModifierContext): JavaModifier[G] =
    modifier match {
  ...
  def convert(
      implicit modifier: ClassOrInterfaceModifierContext
  ): JavaModifier[G] =
    modifier match {
      case ClassOrInterfaceModifier0(annotation) => convert(annotation)
      case ClassOrInterfaceModifier1(name) =>
        name match {
      ...
      case ClassOrInterfaceModifier2(mods) =>
        withModifiers(
          mods,
          m => {
            if (m.consume(m.pure))
              ...
              fail(
                m.nodes.head,
                "This modifier cannot be attached to a declaration in Java",
              )
          },
        )
    }

  def convert(implicit annotation: AnnotationContext): JavaAnnotation[G] =
    annotation match {
      case Annotation0(_, name, annotationArgs) =>
        JavaAnnotation(
          convert(name),
          annotationArgs.map(convert(_)).getOrElse(Seq()),
        )(blame(annotation))
    }

  def convert(
      implicit annotationName: AnnotationNameContext
  ): JavaNamedType[G] =
    annotationName match {
      case AnnotationName0(qualifiedName) =>
        JavaNamedType(convert(qualifiedName).names.map(part => (part, None)))
    }

  def convert(
      implicit annotationArgs: AnnotationArgsContext
  ): Seq[(String, Expr[G])] =
    annotationArgs match {
      case AnnotationArgs0(_, None, _) => Seq()
      case AnnotationArgs0(_, Some(AnnotationArgsElems0(pairs)), _) =>
        convert(pairs)
      case AnnotationArgs0(
            _,
            Some(AnnotationArgsElems1(ElementValue1(expr))),
            _,
          ) =>
        Seq(("value", convert(expr)))
      case _ => ??(annotationArgs)
    }

  def convert(
      implicit pairs: ElementValuePairsContext
  ): Seq[(String, Expr[G])] =
    pairs match {
      case ElementValuePairs0(pair) => convert(pair).toSeq
      case ElementValuePairs1(more, _, pair) =>
        convert(more) ++ convert(pair).toSeq
      case _ => ???
    }

  def convert(
      implicit pair: ElementValuePairContext
  ): Option[(String, Expr[G])] =
    pair match {
      case ElementValuePair0(name, _, ElementValue0(expr)) =>
        logger.warn(
          s"Annotation array initializer at ${OriginProvider(pair).shortPositionText} is discarded"
        )
        None
      case ElementValuePair0(name, _, ElementValue1(expr)) =>
        Some((convert(name), convert(expr)))
      case x => ??(x)
    }

  def convert(implicit modifier: VariableModifierContext): JavaModifier[G] =
    modifier match {
      case VariableModifier0(_) => JavaFinal()
      case VariableModifier1(annotation) => convert(annotation)
    }

  def convert(implicit args: TypeParametersContext): Seq[Variable[G]] =
    args match { case TypeParameters0(_, args, _) => convert(args) }

  def convert(implicit args: TypeParameterListContext): Seq[Variable[G]] =
    args match {
      case TypeParameterList0(arg) => Seq(convert(arg))
      case TypeParameterList1(arg, _, args) => convert(arg) +: convert(args)
    }

  def convert(implicit arg: TypeParameterContext): Variable[G] =
    arg match {
      case TypeParameter0(id, bound) =>
        bound match {
          case None =>
            new Variable(TType(Java.JAVA_LANG_OBJECT))(
              origin(arg).sourceName(convert(id))
            )
          case Some(TypeParameterBound0(_, TypeBound0(ext))) =>
            new Variable(TType(convert(ext)))(
              origin(arg).sourceName(convert(id))
            )
  ...
  def convert(
      implicit decl: ClassBodyDeclarationContext
  ): Seq[ClassDeclaration[G]] =
    decl match {
      case ClassBodyDeclaration0(_) => Nil
      case ClassBodyDeclaration1(isStatic, body) =>
        Seq(new JavaSharedInitialization(isStatic.nonEmpty, convert(body)))
      case ClassBodyDeclaration2(contract, mods, decl) =>
        withContract(contract, c => { convert(decl, mods.map(convert(_)), c) })
      case ClassBodyDeclaration3(inner) => convert(inner)
    }

  def convert(
      implicit decl: InterfaceBodyDeclarationContext
  ): Seq[ClassDeclaration[G]] =
    decl match {
  ...
  def convert(
      implicit decl: MemberDeclarationContext,
      mods: Seq[JavaModifier[G]],
      c: ContractCollector[G],
  ): Seq[ClassDeclaration[G]] =
    decl match {
      case MemberDeclaration0(
            MethodDeclaration0(returnType, name, params, dims, signals, body)
          ) =>
        Seq(
          new JavaMethod(
            mods,
            ...
            convert(params),
            Nil,
            signals.map(convert(_)).getOrElse(Nil),
            convert(body),
            c.consumeApplicableContract(blame(decl)),
          )(blame(decl))
        )
      case MemberDeclaration1(
            GenericMethodDeclaration0(
              typeParams,
              MethodDeclaration0(returnType, name, params, dims, signals, body),
            )
          ) =>
      ...
      case MemberDeclaration3(
            ConstructorDeclaration0(
              name,
              params,
              signals,
              ConstructorBody0(body),
            )
          ) =>
        Seq(
          new JavaConstructor(
      ...
      case MemberDeclaration4(
            GenericConstructorDeclaration0(
              typeParams,
              ConstructorDeclaration0(
                name,
                params,
      ...
      case MemberDeclaration5(interface) =>
        fail(interface, "Inner interfaces are not supported.")
      case MemberDeclaration6(annotation) =>
  ...
  def convert(
      implicit decl: InterfaceMemberDeclarationContext,
      mods: Seq[JavaModifier[G]],
      c: ContractCollector[G],
  ): Seq[ClassDeclaration[G]] =
    decl match {
      case InterfaceMemberDeclaration0(ConstDeclaration0(t, decls, _)) =>
        // JLS SE 7 - 9.3
        Seq(new JavaFields(
          Seq(JavaPublic[G](), JavaStatic[G](), JavaFinal[G]()) ++ mods,
          convert(t),
          convert(decls),
        ))
      case InterfaceMemberDeclaration1(
            InterfaceMethodDeclaration0(t, name, params, dims, signals, _)
          ) =>
        // JLS SE 7 - 9.4
        Seq(
          new JavaMethod(
            Seq(JavaPublic[G](), JavaAbstract[G]()) ++ mods,
            convert(t),
            dims.map(convert(_)).getOrElse(0),
            convert(name),
            convert(params),
      ...
      case InterfaceMemberDeclaration2(
            GenericInterfaceMethodDeclaration0(
              typeParams,
              InterfaceMethodDeclaration0(t, name, params, dims, signals, _),
            )
          ) =>
      ...
      case InterfaceMemberDeclaration3(interface) =>
        fail(interface, "Inner interfaces are not supported.")
      case InterfaceMemberDeclaration4(annotation) =>
        fail(annotation, "Annotations are not supported.")
  ...
  def convert(
      implicit decls: VariableDeclaratorsContext
  ): Seq[JavaVariableDeclaration[G]] =
    decls match {
      case VariableDeclarators0(decl) => Seq(convert(decl))
      case VariableDeclarators1(decl, _, decls) =>
        convert(decl) +: convert(decls)
    }

  def convert(
      implicit decl: VariableDeclaratorContext
  ): JavaVariableDeclaration[G] =
    decl match {
  ...
  def convert(
      implicit decls: ConstantDeclaratorListContext
  ): Seq[JavaVariableDeclaration[G]] =
    decls match {
      case ConstantDeclaratorList0(decl) => Seq(convert(decl))
      case ConstantDeclaratorList1(decl, _, decls) =>
        convert(decl) +: convert(decls)
    }

  def convert(
      implicit decl: ConstantDeclaratorContext
  ): JavaVariableDeclaration[G] =
    decl match {
  ...
  def convert(implicit params: FormalParametersContext): Seq[JavaParam[G]] =
    params match {
      case FormalParameters0(_, params, _) =>
        params.map(convert(_)).getOrElse(Nil)
    }

  def convert(implicit params: FormalParameterListContext): Seq[JavaParam[G]] =
    params match {
  ...
  def convert(
      implicit params: InitFormalParameterListContext
  ): Seq[JavaParam[G]] =
    params match {
      case InitFormalParameterList0(param) => Seq(convert(param))
      case InitFormalParameterList1(param, _, params) =>
        convert(param) +: convert(params)
    }

  def convert(implicit param: FormalParameterContext): JavaParam[G] =
    param match {
      case FormalParameter0(modifiers, tNode, nameDims) =>
        ...
        new JavaParam(modifiers.map(convert(_)), name, t)(
          origin(param).sourceName(name)
        )
    }

  def convert(implicit dims: DimsContext): Int =
    dims match { case Dims0(dims) => dims.size }

  def convert(implicit names: QualifiedNameListContext): Seq[JavaName[G]] =
    names match {
      case QualifiedNameList0(name) => Seq(convert(name))
      case QualifiedNameList1(name, _, names) => convert(name) +: convert(names)
    }

  def convert(implicit name: QualifiedNameContext): JavaName[G] =
    name match {
      case QualifiedName0(id) => JavaName(Seq(convert(id)))
      case QualifiedName1(id, _, names) =>
        JavaName(convert(id) +: convert(names).names)
    }

  def convert(implicit ts: IntExtContext): Seq[Type[G]] =
    ts match { case IntExt0(_, ts) => convert(ts) }

  def convert(implicit t: ExtContext): Type[G] =
    t match { case Ext0(_, t) => convert(t) }

  def convert(implicit ts: ImpContext): Seq[Type[G]] =
    ts match { case Imp0(_, ts) => convert(ts) }

  def convert(implicit ts: TypeListContext): Seq[Type[G]] =
    ts match {
      case TypeList0(t) => Seq(convert(t))
      case TypeList1(t, _, ts) => convert(t) +: convert(ts)
    }

  def convert(implicit signals: ThrowyContext): Seq[JavaNamedType[G]] =
    signals match {
      case Throwy0(_, ts) =>
        convert(ts)
          .map(name => JavaNamedType(name.names.map(part => (part, None))))
    }

  def convert(implicit stat: MethodBodyOrEmptyContext): Option[Statement[G]] =
    stat match {
      case MethodBodyOrEmpty0(_) => None
      case MethodBodyOrEmpty1(MethodBody0(body)) => Some(convert(body))
    }

  def convert(implicit stat: BlockContext): Statement[G] =
    stat match {
      case Block0(_, stats, _) => Scope(Nil, Block(stats.map(convert(_))))
    }

  def convert(implicit stat: BlockStatementContext): Statement[G] =
    stat match {
  ...
  def convert(
      implicit decl: LocalVariableDeclarationStatementContext
  ): Statement[G] =
    decl match {
      case LocalVariableDeclarationStatement0(decl, _) => convert(decl)
    }

  def convert(implicit decl: LocalVariableDeclarationContext): Statement[G] =
    decl match {
      case LocalVariableDeclaration0(mods, t, decls) =>
        JavaLocalDeclarationStatement(new JavaLocalDeclaration(
          mods.map(convert(_)),
          convert(t),
          convert(decls),
        ))
  ...
  def convert(implicit stat: ElseBlockContext): Statement[G] =
    stat match { case ElseBlock0(_, stat) => convert(stat) }

  def convert(
      prefix: LoopAmalgamationContext,
      f: ContractCollector[G] => Statement[G],
      labels: Seq[LabelDecl[G]],
  ...
  def convert(
      prefix: LoopAmalgamationContext,
      f: ContractCollector[G] => Statement[G],
  ): Statement[G] = convert(prefix, f, Nil, new ContractCollector())
  ...
  def convert(implicit stat: StatementContext): Statement[G] =
    stat match {
      ...
      case Statement2(_, cond, body, otherwise) =>
        Branch(
          Seq((convert(cond), convert(body))) ++
            (otherwise match {
              case None => Nil
              case Some(otherwise) =>
                Seq((BooleanValue(true), convert(otherwise)))
            })
        )
      ...
      case Statement6(_, attempt, grab, eventually) =>
        TryCatchFinally(
          convert(attempt),
          eventually.map(convert(_)).getOrElse(Block(Nil)),
          grab.map(convert(_)),
        )
      ...
      case Statement9(_, expr, _, casedStatements, trailingCases, _) =>
        Switch(
          convert(expr),
          Block(
            casedStatements.flatMap(convert(_)) ++ trailingCases.map(convert(_))
          ),
        )
      case Statement10(_, obj, inner) =>
        Synchronized(convert(obj), convert(inner))(blame(stat))
      case Statement11(_, expr, _) =>
        Return(expr.map(convert(_)).getOrElse(Void()))
      case Statement12(_, exc, _) => Throw(convert(exc))(blame(stat))
      case Statement13(_, label, _) =>
        Break(label.map(convert(_)).map(new UnresolvedRef[G, LabelDecl[G]](_)))
      case Statement14(_, label, _) =>
        Continue(
          label.map(convert(_)).map(new UnresolvedRef[G, LabelDecl[G]](_))
        )
  ...
  def convert(implicit label: LoopLabelContext): LabelDecl[G] =
    label match {
      case LoopLabel0(name, _) =>
        new LabelDecl()(origin(label).sourceName(convert(name)))
    }

  def convert(
      implicit switchBlock: SwitchBlockStatementGroupContext
  ): Seq[Statement[G]] =
    switchBlock match {
  ...
  def convert(implicit stat: ForInitContext): Statement[G] =
    stat match {
      case ForInit0(locals) => convert(locals)
      case ForInit1(exprs) => Block(convert(exprs).map(Eval(_)))
    }

  def convert(implicit stat: ForUpdateContext): Statement[G] =
    stat match { case ForUpdate0(exprs) => Block(convert(exprs).map(Eval(_))) }

  def convert(implicit stat: FinallyBlockContext): Statement[G] =
    stat match { case FinallyBlock0(_, block) => convert(block) }

  def convert(implicit grab: CatchClauseContext): CatchClause[G] =
    grab match {
      case CatchClause0(_, _, _mods, ts, id, _, body) =>
        CatchClause(
          new Variable(convert(ts))(origin(grab).sourceName(convert(id))),
          convert(body),
        )
    }

  def convert(implicit ts: CatchTypeContext): TUnion[G] =
    ts match {
      case CatchType0(name) => TUnion(Seq(convert(name)))
      case CatchType1(name, _, names) =>
        TUnion(convert(name) +: convert(names).types)
    }

  def convert(implicit ts: NonWildcardTypeArgumentsContext): Seq[Type[G]] =
    ts match { case NonWildcardTypeArguments0(_, ts, _) => convert(ts) }

  def convert(implicit t: TypeOrVoidContext): Type[G] =
    t match {
      case TypeOrVoid0(_) => TVoid()
      case TypeOrVoid1(t) => convert(t)
    }

  def convert(implicit t: TypeContext): Type[G] =
    t match {
      case Type0(inner) => convert(inner)
      case Type1(element, dims) =>
        FuncTools.repeat(
          TArray[G](_),
          dims.map(convert(_)).getOrElse(0),
          convert(element),
        )
      case Type2(element, dims) =>
        FuncTools.repeat(
          TArray[G](_),
          dims.map(convert(_)).getOrElse(0),
          convert(element),
  ...
  def convert(implicit t: ClassOrInterfaceTypeContext): JavaNamedType[G] =
    t match {
      ...
      case ClassOrInterfaceType1(names, _, name, args) =>
        JavaNamedType(
          convert(names).names :+ (convert(name), args.map(convert(_)))
        )
    }

  def convert(implicit t: CreatedNameContext): Type[G] =
    t match {
      case CreatedName0(t) => convert(t)
      case CreatedName1(t) => convert(t)
    }

  def convert(implicit t: ClassTypeDiamondListContext): JavaNamedType[G] =
    t match {
      ...
      case ClassTypeDiamondList1(name, typeArgs, _, more) =>
        JavaNamedType(
          (convert(name), typeArgs.map(convert(_))) +: convert(more).names
        )
    }

  def convert(implicit ts: TypeArgumentsOrDiamondContext): Seq[Type[G]] =
    ts match {
      case TypeArgumentsOrDiamond0(_, _) => Nil
      case TypeArgumentsOrDiamond1(ts) => convert(ts)
    }

  def convert(implicit t: PrimitiveTypeContext): Type[G] =
    t match {
  ...
  def convert(implicit ts: TypeArgumentsContext): Seq[Type[G]] =
    ts match { case TypeArguments0(_, ts, _) => convert(ts) }

  def convert(implicit ts: TypeArgumentListContext): Seq[Type[G]] =
    ts match {
      case TypeArgumentList0(arg) => Seq(convert(arg))
      case TypeArgumentList1(arg, _, args) => convert(arg) +: convert(args)
    }

  def convert(implicit t: TypeArgumentContext): Type[G] =
    t match {
  ...
  def convert(implicit expr: VariableDeclaratorInitContext): Expr[G] =
    expr match { case VariableDeclaratorInit0(_, init) => convert(init) }

  def convert(implicit expr: VariableInitializerContext): Expr[G] =
    expr match {
      case VariableInitializer0(arrayInit) => convert(arrayInit)
      case VariableInitializer1(expr) => convert(expr)
    }

  def convert(implicit expr: ArrayInitializerContext): Expr[G] =
    expr match {
      case ArrayInitializer0(_, _) => JavaLiteralArray(Nil)
      case ArrayInitializer1(_, exprs, _, _) => JavaLiteralArray(convert(exprs))
    }

  def convert(implicit expr: ParExpressionContext): Expr[G] =
    expr match { case ParExpression0(_, expr, _) => convert(expr) }

  def convert(implicit exprs: ExpressionListContext): Seq[Expr[G]] =
    exprs match {
      case ExpressionList0(expr) => Seq(convert(expr))
      case ExpressionList1(expr, _, exprs) => convert(expr) +: convert(exprs)
    }

  def convert(implicit exprs: VariableInitializerListContext): Seq[Expr[G]] =
    exprs match {
      case VariableInitializerList0(expr) => Seq(convert(expr))
      case VariableInitializerList1(expr, _, exprs) =>
        convert(expr) +: convert(exprs)
    }

  def convert(implicit exprs: SpecifiedDimsContext): Seq[Expr[G]] =
    exprs match {
      case SpecifiedDims0(dim) => Seq(convert(dim))
      case SpecifiedDims1(dim, dims) => convert(dim) +: convert(dims)
    }

  def convert(implicit exprs: ArgumentsContext): Seq[Expr[G]] =
    exprs match {
      case Arguments0(_, exprs, _) => exprs.map(convert(_)).getOrElse(Nil)
    }

  def convert(implicit expr: ConstantExpressionContext): Expr[G] =
    expr match { case ConstantExpression0(expr) => convert(expr) }

  def convert(implicit expr: StatementExpressionContext): Expr[G] =
    expr match { case StatementExpression0(expr) => convert(expr) }

  def convert(implicit expr: SpecifiedDimContext): Expr[G] =
    expr match { case SpecifiedDim0(_, expr, _) => convert(expr) }
  ...
  def convert(implicit expr: ExprContext): Expr[G] =
    expr match {
      ...
      case parse.JavaDeref(obj, _, field) =>
        col.JavaDeref(convert(obj), convert(field))(blame(expr))
      ...
      case JavaGenericInvocation(
            obj,
            _,
            ExplicitGenericInvocation0(typeArgs, invocation),
          ) =>
        convert(invocation, Some(convert(obj)), convert(typeArgs))
      case JavaSubscript(ar, _, idx, _) =>
        AmbiguousSubscript(convert(ar), convert(idx))(blame(expr))
      ...
      case JavaValPostfix(expr, PostfixOp0(valPostfix)) =>
        convert(expr, valPostfix, convert(expr))
      case JavaNew(
            _,
            Some(_),
            Creator1(createdName, creatorRest),
            None,
            None,
          ) =>
      ...
      case JavaPostfixIncDec(inner, postOp) =>
        ...
        postOp match {
          case "++" =>
            PostAssignExpression(target, Plus(target, const(1)))(blame(expr))
          case "--" =>
            PostAssignExpression(target, Minus(target, const(1)))(blame(expr))
        }
      case JavaPrefixOp(preOp, inner) =>
        val target = convert(inner)
        preOp match {
          case "+" =>
            target // TODO PB: not sure if this is true for IEEE floats
          case "-" => UMinus(target)
          case "++" =>
            PreAssignExpression(target, Plus(target, const(1)))(blame(expr))
          case "--" =>
            PreAssignExpression(target, Minus(target, const(1)))(blame(expr))
        }
        ...
        mul match {
          case MulOp0(op) =>
            op match {
              ...
              case "/" => AmbiguousTruncDiv(left, right)(blame(expr))
              case "%" => AmbiguousTruncMod(left, right)(blame(expr))
            }
          case MulOp1(specOp) => convert(expr, specOp, left, right)
        }
      case JavaAdd(left, op, right) =>
        op match {
      ...
      case JavaBitXor(left, _, right) =>
        AmbiguousComputationalXor(convert(left), convert(right))
      case JavaBitOr(left, _, right) =>
        AmbiguousComputationalOr(convert(left), convert(right))
      case JavaAnd(left, and, right) =>
        and match {
      ...
      case JavaValImp(left, imp, right) =>
        imp match {
          case ImpOp0(specOp) =>
            convert(expr, specOp, convert(left), convert(right))
        }
  ...
  def convertBipGlue(implicit p: ParserRuleContext): JavaBipGlue[G] =
    p match {
       ...
       */
      case CreatorRest1(
            ClassCreatorRest0(Arguments0(_, None, _), Some(classBody))
          ) =>
        convertBipGlue(classBody)
      case ClassBody0(_, Seq(decl), _) => convertBipGlue(decl)
      case ClassBodyDeclaration2(None, _, MemberDeclaration0(method)) =>
        convertBipGlue(method)
      case MethodDeclaration0(
            ...
            None,
            None,
            MethodBodyOrEmpty1(m @ MethodBody0(Block0(_, statements, _))),
          ) =>
  ...
  object JavaId {
    def unapply(javaIdentifier: JavaIdentifierContext): Option[String] =
      Some(javaIdentifier.start.getText)
  }

  object Arguments {
    def unapply(arguments: ArgumentsContext): Option[Seq[ExpressionContext]] =
      arguments match {
        case Arguments0("(", None, ")") => Some(Nil)
        case Arguments0("(", Some(exprList), ")") => Some(getExprs(exprList))
      }

    def getExprs(exprList: ExpressionListContext): Seq[ExpressionContext] =
      exprList match {
        case ExpressionList0(expression) => Seq(expression)
        case ExpressionList1(expression, _, exprList) =>
          expression +: getExprs(exprList)
      }
  }

  object CleanPrimary {
    def unapply(ann: ParserRuleContext): Option[PrimaryContext] =
      ann match {
        case Expression0(
              None,
              parse.JavaPrimary(AnnotatedPrimary0(None, primary, None)),
              None,
            ) =>
          Some(primary)
        case parse.JavaPrimary(AnnotatedPrimary0(None, primary, None)) =>
          Some(primary)
  ...
  def convertDotClassToType(implicit e: ExpressionContext): Type[G] =
    e match { case CleanPrimary(Primary6(t, _, _)) => convert(t) }

  def convertPorts(args: Seq[ExpressionContext]): Seq[JavaBipGlueName[G]] = {
    assert((args.length % 2) == 0)
    args.grouped(2).map { case Seq(portType, portName) =>
      JavaBipGlueName(convertDotClassToType(portType), convert(portName))(
        origin(portType)
      )
    }.toSeq
  }

  def convertPort(implicit e: ExprContext): Option[JavaBipGlueName[G]] =
    e match {
      case CleanPrimary(
            Primary5(
              JavaIdentifier0("port"),
              None,
              Arguments(Seq(portType, portName)),
              None,
              None,
            )
          ) =>
  ...
  def convertSynchron(implicit e: ExprContext): Option[JavaBipGlueName[G]] =
    e match {
      case CleanPrimary(
            Primary5(
              JavaIdentifier0("synchron"),
              None,
              Arguments(Seq(portType, portName)),
              None,
              None,
  ...
  def convertData(implicit e: ExprContext): Option[JavaBipGlueName[G]] =
    e match {
      case CleanPrimary(
            Primary5(
              JavaIdentifier0("data"),
              None,
              Arguments(Seq(portType, portName)),
              None,
              None,
  ...
  def convertBipGlueElement(
      implicit s: ParserRuleContext
  ): JavaBipGlueElement[G] =
    s match {
      case BlockStatement1(statement) => convertBipGlueElement(statement)
      case Statement16(StatementExpression0(expr), _) =>
        convertBipGlueElement(expr)
      case Expression0(None, expr, None) => convertBipGlueElement(expr)
      case parse.JavaInvocation(
            obj,
            _,
            JavaIdentifier0("requiresNothing"),
            None,
            Arguments(Seq()),
            None,
            None,
      ...
      case parse.JavaInvocation(
            obj,
            _,
            JavaIdentifier0("acceptsNothing"),
            None,
            Arguments(Seq()),
            None,
            None,
      ...
      case parse.JavaInvocation(
            ...
            JavaId("to"),
            None,
            args @ Arguments(Seq(portType, portName)),
            None,
            None,
          ) =>
        (convertSynchron(obj), convertData(obj)) match {
          case (Some(synchronPortName), _) =>
            JavaBipGlueSynchron(
              synchronPortName,
              JavaBipGlueName(
                convertDotClassToType(portType),
                convert(portName),
              )(origin(args)),
            )
  ...
  def convert(
      implicit invocation: ExplicitGenericInvocationSuffixContext,
      obj: Option[Expr[G]],
      typeArgs: Seq[Type[G]],
  ): Expr[G] =
    invocation match {
  ...
  def convert(
    ...
    expr match {
      ...
      case Creator1(name, creator) =>
        creator match {
          case CreatorRest0(array) => convert(array, convert(name))
          case CreatorRest1(cls) =>
            convert(cls, Nil, convert(name), givenArgs, yields)
        }
    }

  def convert(
      implicit creator: ClassCreatorRestContext,
      ts: Seq[Type[G]],
      name: Type[G],
      givenArgs: Seq[(Ref[G, Variable[G]], Expr[G])],
      yields: Seq[(Expr[G], Ref[G, Variable[G]])],
  ...
  def convert(
      implicit creator: ArrayCreatorRestContext,
      name: Type[G],
  ): Expr[G] =
    creator match {
      case ArrayCreatorRest0(dims, init) =>
        JavaNewLiteralArray(name, convert(dims), convert(init))(blame(creator))
      case ArrayCreatorRest1(specDims, extraDims) =>
        JavaNewDefaultArray(
          name,
          convert(specDims),
          extraDims.map(convert(_)).getOrElse(0),
        )(blame(creator))
    }

  def convert(implicit expr: AnnotatedPrimaryContext): Expr[G] =
    expr match {
      case AnnotatedPrimary0(pre, inner, post) =>
        convertEmbedWith(pre, convertEmbedThen(post, convert(inner)))
    }

  def convert(implicit expr: PrimaryContext): Expr[G] =
    expr match {
  ...
  def convert(implicit expr: LiteralContext): Expr[G] =
    expr match {
  ...
  def convert(implicit id: VariableDeclaratorIdContext): (String, Int) =
    id match {
      case VariableDeclaratorId0(name, Some(dims)) =>
        (convert(name), convert(dims))
      case VariableDeclaratorId0(name, None) => (convert(name), 0)
    }

  def convert(implicit id: JavaIdentifierContext): String =
    id match {
      case JavaIdentifier0(text) => text
      case JavaIdentifier1(inner) => convert(inner)
    }

  def convert(implicit decl: LangGlobalDeclContext): Seq[GlobalDeclaration[G]] =
    decl match { case LangGlobalDecl0(typeDecl) => convert(typeDecl) }

  def convert(implicit decl: LangClassDeclContext): Seq[ClassDeclaration[G]] =
    decl match { case LangClassDecl0(classDecl) => convert(classDecl) }

  def convert(implicit stat: LangStatementContext): Statement[G] =
    stat match { case LangStatement0(block) => convert(block) }

  def convert(implicit t: LangTypeContext): Type[G] =
    t match { case LangType0(t) => convert(t) }

  def convert(implicit e: LangExprContext): Expr[G] =
    e match { case LangExpr0(e) => convert(e) }

  def convert(implicit id: LangIdContext): String =
  ...
  def convert[T](
    ...
    decl match {
      ...
      case ValInstanceOperatorFunction(
            contract,
            modifiers,
            "pure",
            t,
      ...
      case ValInstanceOperatorMethod(
            contract,
            modifiers,
            t,
            name,
  ...
  def convert(implicit e: ValPrimaryContext): Expr[G] =
    e match {
      ...
      case ValHidden(_,_, expr, _, perm, _) => Hidden(convert(expr), convert(perm))
      case ValLeakable(_, _, expr, _) => Leakable(convert(expr))
    }
