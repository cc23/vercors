...
case class PVLToCol[G](
  ...
  def convert(implicit program: ProgramContext): Seq[GlobalDeclaration[G]] =
    program match { case Program0(decls, _, _) => decls.flatMap(convert(_)) }

  def convert(implicit decl: ProgramDeclContext): Seq[GlobalDeclaration[G]] =
    decl match {
  ...
  def convertConstants(
      implicit identifierList: IdentifierListContext
  ): Seq[EnumConstant[G]] =
    identifierList match {
      case IdentifierList0(id) =>
        Seq(new vct.col.ast.EnumConstant[G]()(origin(identifierList).sourceName(
          convert(id)
        )))
      case IdentifierList1(id, _, tail) =>
        new vct.col.ast.EnumConstant[G]()(origin(identifierList).sourceName(
          convert(id)
  ...
  def convertProcedure(implicit method: MethodContext): Procedure[G] =
    method match {
      case Method0(
            contract,
            modifiers,
  ...
  def typeVar(identifier: IdentifierContext): Variable[G] =
    new Variable(TType(TAnyValue()))(
      origin(identifier).sourceName(convert(identifier))
    )

  def convertVars(implicit ids: IdentifierListContext): Seq[Variable[G]] =
    ids match {
      case IdentifierList0(identifier) => Seq(typeVar(identifier))
      case IdentifierList1(identifier, _, identifiers) =>
        typeVar(identifier) +: convertVars(identifiers)
    }

  def convert(implicit cls: DeclClassContext): GlobalDeclaration[G] =
    cls match {
  ...
  def convert(implicit decl: ClassDeclContext): Seq[ClassDeclaration[G]] =
    decl match {
  ...
  def convert(implicit method: MethodContext): InstanceMethod[G] =
    method match {
      case Method0(
            contract,
            modifiers,
  ...
  def convert(implicit body: MethodBodyContext): Option[Statement[G]] =
    body match {
  ...
  def convert(
      implicit constructor: ConstructorContext
    ...
    constructor match {
      case Constructor0(contract, _, typeVars, _, args, _, body) =>
        Seq(withContract(
          ...
          contract =>
            new PVLConstructor(
              contract.consumeApplicableContract(blame(constructor)),
              typeVars.map(convert(_)).getOrElse(Seq()),
              args.map(convert(_)).getOrElse(Nil),
              convert(body),
            )(blame(constructor)),
        ))
  ...
  def convert(implicit finalFlag: FinalFlagContext): FieldFlag[G] =
    finalFlag match { case FinalFlag0(_) => Final() }

  def convert(implicit field: FieldContext): Seq[InstanceField[G]] =
    field match {
      case Field0(finalFlag, t, ids, _) =>
        convert(ids).map(name =>
          new InstanceField[G](convert(t), finalFlag.map(convert(_)).toSeq)(
            origin(field).sourceName(name)
          )
        )
    }

  def convert(implicit method: RunMethodContext): Seq[RunMethod[G]] =
    method match {
  ...
  def convert(implicit method: VesuvEntryContext): Seq[VeSUVMainMethod[G]] =
    method match {
      case VesuvEntry0(_, maybeBody) =>
        Seq(new VeSUVMainMethod(convert(maybeBody))(blame(method)))
    }

  def convert(implicit args: ArgsContext): Seq[Variable[G]] =
    args match {
      case Args0(t, name) =>
        Seq(new Variable(convert(t))(origin(name).sourceName(convert(name))))
      case Args1(t, name, _, args) =>
        new Variable(convert(t))(origin(name).sourceName(convert(name))) +:
          convert(args)
    }

  def convert(implicit exprs: ExprListContext): Seq[Expr[G]] =
    exprs match {
      case ExprList0(e) => Seq(convert(e))
      case ExprList1(e, _, es) => convert(e) +: convert(es)
    }

  def convert(implicit tuple: TupleContext): Seq[Expr[G]] =
    tuple match {
      case Tuple0(_, exprs, _) => exprs.map(convert(_)).getOrElse(Nil)
    }

  def convert(implicit exprs: NewDimsContext): Seq[Expr[G]] =
    exprs match { case NewDims0(dims) => dims.map(convert(_)) }

  def convert(implicit dim: QuantifiedDimContext): Expr[G] =
    dim match { case QuantifiedDim0(_, inner, _) => convert(inner) }
  ...
  def convert(implicit expr: SetExprContext): Expr[G] =
    expr match {
      case SetExpr0(x, _, xs) => AmbiguousMember(convert(x), convert(xs))
      case SetExpr1(inner) => convert(inner)
    }

  def convert(implicit expr: AddExprContext): Expr[G] =
    expr match {
      case AddExpr0(left, _, right) =>
        AmbiguousPlus(convert(left), convert(right))(blame(expr))
      case AddExpr1(left, _, right) =>
        AmbiguousMinus(convert(left), convert(right))(blame(expr))
      case AddExpr2(inner) => convert(inner)
    }

  def convert(implicit expr: MultExprContext): Expr[G] =
    expr match {
      case MultExpr0(left, _, right) =>
        AmbiguousMult(convert(left), convert(right))
      case MultExpr1(left, _, right) =>
        AmbiguousDiv(convert(left), convert(right))(blame(expr))
      case MultExpr2(left, _, right) =>
        AmbiguousMod(convert(left), convert(right))(blame(expr))
      case MultExpr3(left, specOp, right) =>
        convert(expr, specOp, convert(left), convert(right))
      case MultExpr4(inner) => convert(inner)
    }

  def convert(implicit expr: PowExprContext): Expr[G] =
    expr match {
      case PowExpr0(left, _, right) => Exp(convert(left), convert(right))
      case PowExpr1(inner) => convert(inner)
    }

  def convert(implicit expr: SeqAddExprContext): Expr[G] =
    expr match {
      case SeqAddExpr0(left, _, right) => Cons(convert(left), convert(right))
      case SeqAddExpr1(left, _, right) => ??(expr)
      case SeqAddExpr2(m, _, _, k, _, v, _) =>
        MapCons(convert(m), convert(k), convert(v))
      case SeqAddExpr3(inner) => convert(inner)
    }

  def convert(implicit expr: UnaryExprContext): Expr[G] =
    expr match {
  ...
  def convert(implicit expr: NewExprContext): Expr[G] =
    expr match {
      ...
      case NewExpr1(_, t, dims) =>
        NewArray(convert(t), convert(dims), moreDims = 0, true)(blame(expr))
      case NewExpr2(inner) => convert(inner)
    }

  def convert(implicit expr: PostfixExprContext): Expr[G] =
    expr match {
      case PostfixExpr0(obj, _, field, None) =>
        PVLDeref(convert(obj), convert(field))(blame(expr))
      case PostfixExpr0(
            obj,
            _,
            field,
  ...
  def convert(implicit expr: UnitContext): Expr[G] =
    expr match {
      ...
      case PvlDecimalF(s"${n}f") /* take off final "f" */ =>
        FloatValue(BigDecimal(n), PVL.float32)
  ...
  def convert(implicit stat: StatementContext): Statement[G] =
    stat match {
      case PvlReturn(_, value, _) =>
        Return(value.map(convert(_)).getOrElse(Void()))
      ...
      case PvlIf(_, _, cond, _, body, Some(ElseBlock0(_, otherwise))) =>
        PVLBranch(
          Seq((convert(cond), convert(body)), (tt[G], convert(otherwise)))
        )(blame(stat))
      case PvlBarrier(_, _, block, tags, _, body) =>
        val (contract, content: Statement[G]) =
          body match {
      ...
      case PvlPar(impl) => ParStatement(convert(impl))
      case PvlVec(_, _, iter, _, body) => ??(stat)
      ...
      case PvlAtomic(_, _, invs, _, body) =>
        ParAtomic(
          convert(invs).map(new UnresolvedRef[G, ParInvariantDecl[G]](_)),
          convert(body),
        )(blame(stat))
  ...
  def convertCommunicate(
      inv: Option[ChannelInvariantContext],
      to: AccessContext,
      from: AccessContext,
  ...
  def convertList(implicit stat: ForStatementListContext): Seq[Statement[G]] =
    stat match {
      case ForStatementList0(stat) => Seq(convert(stat))
      case ForStatementList1(stat, _, stats) =>
        convert(stat) +: convertList(stats)
    }

  def convert(implicit stat: AllowedForStatementContext): Statement[G] =
    stat match {
      ...
      case PvlIncDec(name, op) =>
        ...
        Eval(op match {
          case "++" =>
            PostAssignExpression[G](target, target + const(1))(blame(stat))
          case "--" =>
            PostAssignExpression[G](target, target - const(1))(blame(stat))
        })
      case PvlAssign(target, _, value) =>
  ...
  def convert(implicit region: ParRegionContext): ParRegion[G] =
    region match {
      case PvlParallel(_, _, regions, _) =>
        ParParallel(regions.map(convert(_)))(blame(region))
      case PvlSequential(_, _, regions, _) =>
        ParSequential(regions.map(convert(_)))(blame(region))
          ...
          contract => {
            val decl =
              name match {
                ...
                case Some(name) =>
                  new ParBlockDecl[G]()(origin(region).sourceName(convert(
                    name
                  )))
              }
            ParBlock(
              ...
              AstBuildHelpers
                .foldStar(contract.consume(contract.context_everywhere)),
              AstBuildHelpers.foldStar(contract.consume(contract.requires)),
              AstBuildHelpers.foldStar(contract.consume(contract.ensures)),
              convert(impl),
            )(blame(region))
  ...
  def convert(implicit iters: ParBlockIterContext): Seq[IterVariable[G]] =
    iters match { case ParBlockIter0(_, iters, _) => convert(iters) }

  def convert(implicit decls: DeclListContext, t: Type[G]): Seq[Statement[G]] =
    decls match {
  ...
  def convert(implicit block: BlockContext): Statement[G] =
    block match {
      case Block0(_, stats, _) => Scope(Nil, Block(stats.map(convert(_))))
    }

  def convert(
      implicit tags: BarrierTagsContext
  ): Seq[Ref[G, ParInvariantDecl[G]]] =
    tags match {
      case BarrierTags0(_, ids) =>
        convert(ids).map(new UnresolvedRef[G, ParInvariantDecl[G]](_))
    }

  def convert(implicit iters: ItersContext): Seq[IterVariable[G]] =
    iters match {
      case Iters0(iter) => Seq(convert(iter))
      case Iters1(iter, _, iters) => convert(iter) +: convert(iters)
    }

  def convert(implicit iter: IterContext): IterVariable[G] =
    iter match {
  ...
  def convert(implicit wait: ParWaitListContext): Seq[Ref[G, ParBlockDecl[G]]] =
    ??(wait)
  ...
  def convert(implicit dims: TypeDimsContext): Int =
    dims match {
      case TypeDims0(dims) => dims.size
      case TypeDims1(dims) => dims.size
    }

  def convert(implicit t: NonArrayTypeContext): Type[G] =
    t match {
      case NonArrayType0(inner) => convert(inner)
      case NonArrayType1(name) =>
        name match {
          case "int" => TInt()
          case "boolean" => TBool()
          case "void" => TVoid()
  ...
  def convert(implicit t: ClassTypeContext): Type[G] =
    t match {
      case ClassType0(name, typeArgs) =>
        PVLNamedType(convert(name), typeArgs.map(convert(_)).getOrElse(Nil))
    }

  def convert(implicit ts: TypeArgsContext): Seq[Type[G]] =
    ts match { case TypeArgs0(_, ts, _) => convert(ts) }

  def convert(implicit ts: DeclaredTypeArgsContext): Seq[Variable[G]] =
    ts match {
  ...
  def convert(implicit id: IdentifierContext): String =
    id match {
      case Identifier0(id) => id
      case Identifier1(id) => id.substring(1, id.length - 1)
    }

  def withContract[T](node: ContractContext, f: ContractCollector[G] => T): T =
    node match {
