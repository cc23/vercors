...
case class InsufficientPermissionToExhale(node: Expr[_])
  ...
  override def inlineDescCompletion: String =
    s"there might be insufficient permission for ${node.o.inlineContextText}"
}
...
case class AssignFieldFailed(node: SilverFieldAssign[_])
...
case class SYCLItemMethodPreconditionFailed(node: InvokingNode[_])
  ...
  override def position: String = node.o.shortPositionText
  override def descInContext: String =
    "The dimension parameter should be greater or equal to zero and smaller than the number of dimensions in the (nd_)item."
  override def inlineDescWithSource(source: String): String =
...
case class AutoValueLeakCheckFailed(
  ...
  override def inlineDescWithSource(node: String, failure: String): String =
    s"Some of the permission from `$node` was leaked before the postcondition was evaluated, since $failure."
}
...
case class VectorDivByZero(node: DividingExpr[_], i: Int) extends DivByZero {
  ...
  override def descInContext: String =
    f"Element $i of the divisor vector may be zero."
  override def inlineDescWithSource(source: String): String =
    s"Element $i of the divisor vector in `$source` may be zero."
}
...
case class SeqAssignInsufficientPermission(node: EndpointStatement[_])
  ...
  override def descInContext: String =
    "There may be insufficient permission to access this field on this endpoint."
  override def inlineDescWithSource(source: String): String =
...
case class ParticipantsNotDistinct(node: Communicate[_])
  ...
  override def descInContext: String =
    "The participants in this communicate statement might not be distinct."
  override def inlineDescWithSource(source: String): String =
...
case class SetEmpty(node: Expr[_]) extends NodeVerificationFailure {
  ...
  override def inlineDescWithSource(source: String): String =
    s"`$source` may be empty."
}

sealed trait VectorBoundFailure extends FrontendSubscriptError with BuiltinError
case class VectorBoundNegative(node: Expr[_])
    extends VectorBoundFailure with NodeVerificationFailure {
  override def code: String = "vecIndexNegative"
  override def descInContext: String =
    "The index in this vector subscript may be negative."
  override def inlineDescWithSource(source: String): String =
    s"The index in `$source` may be negative."
}
case class VectorBoundExceedsLength(node: Expr[_])
    extends VectorBoundFailure with NodeVerificationFailure {
  override def code: String = "vecIndexExceedsLength"
  override def descInContext: String =
    "The index in this vector subscript may exceed the length of the vector."
  override def inlineDescWithSource(source: String): String =
    s"The index in `$source` may exceed the length of the vector."
}
...
case class KernelPostconditionFailed(
    failure: ContractFailure,
    eitherNode: Either[CGpgpuKernelSpecifier[_], CPPLambdaDefinition[_]],
...
case class PointerOffsetNonZero(node: Expr[_])
    extends PointerFreeError with NodeVerificationFailure {
  override def code: String = "ptrOffsetNonZero"
  override def descInContext: String =
    "Free can only be called on pointers which are allocated and are at the start of the pointer block."
  override def inlineDescWithSource(source: String): String =
    s"Pointer in `$source` cannot be freed due to invalid pointer block."
}

case class PointerInsufficientFreePermission(node: Expr[_])
    extends PointerFreeError with NodeVerificationFailure {
  override def code: String = "ptrFreePerm"
  override def descInContext: String =
    "Not enough permission to free the whole pointer block."
  override def inlineDescWithSource(source: String): String =
    s"Pointer in `$source` cannot be freed due to insufficient permission."
}

case class GenericPointerFreeError(node: Expr[_])
    extends PointerFreeError with NodeVerificationFailure {
  override def code: String = "ptrFreeError"
  override def descInContext: String = "Pointer block cannot be freed."
  override def inlineDescWithSource(source: String): String =
    s"Pointer in `$source` cannot be freed."
}

case class PointerInsufficientFreeFieldPermission(node: Expr[_], field: String)
    extends PointerFreeError with NodeVerificationFailure {
  override def code: String = "ptrFreeFieldError"
  override def descInContext: String =
    s"Not enough permission to free the whole pointer block, since we miss permission for field `$field`"
  override def inlineDescWithSource(source: String): String =
    s"Pointer in `$source` cannot be freed."
}
...
case class NontrivialUnsatisfiable(node: ApplicableContract[_])
    extends NodeVerificationFailure with BipConstructorFailure {
...
case class BipOutgoingDataPreconditionUnsatisfiable(node: BipOutgoingData[_])
    extends JavaAnnotationFailure with NodeVerificationFailure {
  ...
  override def desc: String =
    Message.messagesInContext(
      (
        ...
        ) :+ (failure.node.o, s"${failure.descCompletion} $errUrl"): _*
    )
...
object FramedVectorIndex
    extends PanicBlame(
      "access in `∀i. 0 <= i < xs.length ==> ...xs[i]...` should never be out of bounds"
    )
