...
trait SilverBackend
    extends Backend[(silver.Program, Map[Int, col.Node[_]])] with LazyLogging {
  ...
  case class NoInfo(node: silver.Infoed) extends SystemError {
    override def text: String =
      node match {
        ...
        case _ => s"node has no info (`$node`)"
      }
  }
...
//            for (error <- errors) { logger.warn(error.toString) }
  ...
  override def submit(
    ...
    try {
      val transformedProgram =
        plugins.beforeVerify(silverProgram) match {
      ...
      val backendVerifies =
        plugins.mapVerificationResult(
          transformedProgram,
          verifier.verify(transformedProgram),
        ) match {
          case Success => true
          case Failure(errors) =>
      ...
      backendVerifies
    } finally { stopVerifier(verifier) }
  }

  def processError(error: AbstractError): Unit =
    error match {
      case err: AbstractVerificationError =>
        err match {
          case Internal(node, reason, _) =>
            throw ViperCrashed(
              s"Viper returned an internal error at ${Try(node.toString())
                  .getOrElse("?")}: $reason"
            )
              ...
              case fieldAssign @ col.SilverFieldAssign(_, _, _) =>
                reason match {
                  case reasons.InsufficientPermission(access)
                      if get[col.Node[_]](access) == fieldAssign =>
                    fieldAssign.blame
                      .blame(blame.AssignFieldFailed(fieldAssign))
                  case otherReason => defer(otherReason)
                }
              case col.SilverLocalAssign(_, _) => defer(reason)
            }
          case CallFailed(_, reason, _) => defer(reason)
              ...
              case reasons.InsufficientPermission(permNode) =>
                get[col.Node[_]](permNode) match {
                  case _: col.Perm[_] | _: col.PredicateApply[_] |
                      _: col.Value[_] =>
                    exhale.blame
                      .blame(blame.ExhaleFailed(getFailure(reason), exhale))
                  case _ => defer(reason)
                }
          ...
          case InhaleFailed(node, reason, _) => defer(reason)
          ...
          case PostconditionViolated(_, member, reason, _) =>
            ...
            applicable.blame.blame(blame.PostconditionFailed(
              path(reason.offendingNode),
              getFailure(reason),
              applicable,
            ))
          case FoldFailed(node, reason, _) =>
            ...
            reason match {
              case reasons.InsufficientPermission(access) =>
                if (node.contains(access)) { defer(reason) }
                else {
          ...
          case LoopInvariantNotPreserved(node, reason, _) =>
            ...
            invariant.blame.blame(
              blame.LoopInvariantNotMaintained(getFailure(reason), invariant)
            )
          case LoopInvariantNotEstablished(node, reason, _) =>
            val invariant = info(node).invariant.get
            invariant.blame.blame(
              blame.LoopInvariantNotEstablished(getFailure(reason), invariant)
            )
          case FunctionNotWellformed(_, reason, _) => defer(reason)
          ...
          case TerminationFailed(node, reason, cached) =>
            throw NotSupported(
              s"Vercors does not support termination measures from Viper"
            )
          ...
          case ApplyFailed(node, reason, _) =>
            ...
            reason match {
              ...
              case reasons.MagicWandChunkNotFound(magicWand) =>
                applyNode.blame.blame(blame.WandApplyFailed(
                  blame.InsufficientPermissionToExhale(get(magicWand)),
                  applyNode,
                ))
              case _ => defer(reason)
            }
          case MagicWandNotWellformed(_, _, _) =>
            throw NotSupported(
              s"Vercors does not support magic wands from Viper"
            )
  ...
  def getFailure(reason: ErrorReason): blame.ContractFailure =
    reason match {
      case reasons.AssertionFalse(expr) =>
        blame.ContractFalse(get[col.Expr[_]](expr))
      case reasons.InsufficientPermission(access) =>
        blame.InsufficientPermissionToExhale(get[col.Expr[_]](access))
      case reasons.MagicWandChunkNotFound(wand) =>
        blame.InsufficientPermissionToExhale(get[col.Expr[_]](wand))
      case reasons.NegativePermission(p) =>
        blame
  ...
  def getDecreasesClause(reason: ErrorReason): col.DecreasesClause[_] =
    reason match {
      ...
      case TupleDecreasesFalse(node: Infoed) =>
        get[col.DecreasesClause[_]](node)
