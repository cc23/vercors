...
case object EncodeTryThrowSignals extends RewriterBuilder {
  ...
  override def desc: String =
    "Encode try, throw and signals specifications to goto, exception out-parameters and regular postconditions."

  case class ThrowNullAssertFailed(t: Throw[_]) extends Blame[AssertFailed] {
    override def blame(error: AssertFailed): Unit = t.blame.blame(ThrowNull(t))
  }

  case class PackageThrowsAssertFailed(pack: WandPackage[_])
      extends Blame[AssertFailed] {
    override def blame(error: AssertFailed): Unit =
      pack.blame.blame(PackageThrows(pack))
  }

  private def ExcVar: Origin =
    Origin(Seq(PreferredName(Seq("exc")), LabelContext("current exception")))

  private def CurrentlyHandling: Origin =
    Origin(Seq(
  ...
  private def ExcBeforeLoop: Origin =
    Origin(
      Seq(PreferredName(Seq("excBeforeLoop")), LabelContext("loop exception"))
    )

  case class SignalsClosedPostconditionFailed(method: AbstractMethod[_])
  ...
  case class AssertFailedSignalsNotClosed(method: AbstractMethod[_])
      extends Blame[AssertFailed] {
    override def blame(error: AssertFailed): Unit =
      method.blame.blame(ExceptionNotInSignals(method))
  }

  case class SignalsFailedPostconditionFailed(method: AbstractMethod[_])
      extends Blame[PostconditionFailed] {
    override def blame(error: PostconditionFailed): Unit =
...
case class EncodeTryThrowSignals[Pre <: Generation]() extends Rewriter[Pre] {
            ...
            loop.rewrite(contract =
              loop.contract match {
                case inv @ LoopInvariant(invariant, decreases) =>
                  LoopInvariant(
                    getExc === beforeLoop.get &* dispatch(invariant),
  ...
  def inlineExtraCondition(
      condition: Expr[Post],
      clause: AccountedPredicate[Pre],
  ): AccountedPredicate[Post] =
    clause match {
      case UnitAccountedPredicate(pred) =>
        UnitAccountedPredicate[Post](mapUnfoldedStar(
          pred,
          (e: Expr[Pre]) => Implies(condition, dispatch(e))(e.o),
        ))(clause.o)
      case SplitAccountedPredicate(left, right) =>
        SplitAccountedPredicate[Post](
          inlineExtraCondition(condition, left),
          inlineExtraCondition(condition, right),
        )(clause.o)
    }
        ...
        currentException.having(exc) {
          ...
          lazy val ensures: AccountedPredicate[Post] = SplitAccountedPredicate(
            left = UnitAccountedPredicate(
              (exc.get !== Null()) ==> foldOr(method.contract.signals.map {
                case SignalsClause(binding, _) =>
                  InstanceOf(exc.get, TypeValue(dispatch(binding.t)))
              })
            ),
            right = SplitAccountedPredicate(
              left = inlineExtraCondition(
                exc.get === Null(),
                method.contract.ensures,
              ),
              right = UnitAccountedPredicate(
          ...
          allScopes.anyDeclare(allScopes.anySucceedOnly(
            ...
            method.rewrite(
              ...
              outArgs =
                variables.collect {
                  variables.declare(exc); method.outArgs.foreach(dispatch)
                }._1,
              contract = method.contract
                .rewrite(ensures = ensures, signals = Nil),
            ),
          ))
        }
  ...
  override def dispatch(e: Expr[Pre]): Expr[Post] =
    e match {
      case Local(Ref(v))
          if signalsBinding.nonEmpty && signalsBinding.top._1 == v =>
