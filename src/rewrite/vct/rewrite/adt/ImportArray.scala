...
case class ImportArray[Pre <: Generation](importer: ImportADTImporter)
  ...
  private def getArrayField(arr: Expr[Pre]): Ref[Post, SilverField[Post]] = {
    ...
    arrayField.getOrElseUpdate(
      tElement, {
        globalDeclarations
          .declare(new SilverField(dispatch(tElement))(ArrayField(tElement)))
      },
    ).ref
  }
    ...
    e match {
      ...
      case sub @ ArraySubscript(arr, index) =>
        SilverDeref(
          ...
          field = getArrayField(arr),
        )(ArrayFieldInsufficientPermission(sub.blame, sub))
